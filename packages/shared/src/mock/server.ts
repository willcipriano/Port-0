...
import { mockPasswordLevelForNode } from '../world/passwordLevelLabel.js';
...
function withPasswordLevel(node: MockWorldNode) {
  return {
    ...node,
    passwordLevel: mockPasswordLevelForNode(node.ipv6, node.osArchetypeId),
  };
}

const MOCK_WORLD_NODES: MockWorldNode[] = [
...
const server = createServer((req, res) => {
  ...
  if (req.method === 'GET' && path === '/world/nodes') {
    return json(res, 200, {
      nodes: MOCK_WORLD_NODES.map(withPasswordLevel),
    });
  }
